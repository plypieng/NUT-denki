...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 管理者かどうかのチェック - NEXT_PUBLIC_ADMIN_EMAIL を使用
    const isAdmin = session.user?.email === process.env.NEXT_PUBLIC_ADMIN_EMAIL;
    console.log('Admin check in PATCH:', session.user?.email, process.env.NEXT_PUBLIC_ADMIN_EMAIL, isAdmin);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 管理者かどうかのチェック - NEXT_PUBLIC_ADMIN_EMAIL を使用
    const isAdmin = session.user?.email === process.env.NEXT_PUBLIC_ADMIN_EMAIL;
    console.log('Admin check in DELETE:', session.user?.email, process.env.NEXT_PUBLIC_ADMIN_EMAIL, isAdmin);
    ...
    if (!isAdmin) {
      const userEmail = session.user?.email || '';
      console.log('User email:', userEmail);
      console.log('Student owner email:', (existingStudent as any).ownerEmail);
      
      // ユーザーがプロフィールのオーナーでない場合は削除禁止
      // 古いプロフィールでownerEmailが設定されていない場合は学籍番号でチェック
      if ((existingStudent as any).ownerEmail && (existingStudent as any).ownerEmail !== userEmail) {
        // オーナーメールが設定されているがユーザーのメールと一致しない
        return NextResponse.json({ error: "自分のプロフィールのみ削除できます" }, { status: 403 });
      } else if (!(existingStudent as any).ownerEmail) {
        // ownerEmailが設定されていない古いプロフィールの場合は学籍番号でチェック
        const emailUsername = userEmail.split('@')[0] || '';
      ...
      // 既存プロフィールのownerEmailフィールドを更新
      if (!(existingStudent as any).ownerEmail) {
        await prisma.student.update({
          where: { id },
          data: { ownerEmail: userEmail } as any
        });
